...
func (s *GatewayStore) MarkAgentOffline(
	cluster,
	instance string,
	generation uint64,
	lastSeen time.Time,
	...
	_, err := s.db.Exec(`UPDATE agent_status
		...
		formatTime(lastSeen.UTC()), formatTime(now), cluster, instance, generation)
	return err
}
...
func (s *GatewayStore) TouchAgent(cluster, instance string, generation uint64, lastSeen time.Time) error {
	return s.TouchAgentContext(context.Background(), cluster, instance, generation, lastSeen)
}

func (s *GatewayStore) TouchAgentContext(
	ctx context.Context,
	cluster,
	instance string,
	generation uint64,
