...
.cnb.yml                     Temporary CNB main CI and image build (no CD)
...
Run the same checks before publishing source:
...
- Temporarily, CNB verifies `main` pull requests and pushes. After `main` push
  checks pass, it builds and publishes `base-light:20260712` and
  `doops.sh:20260712`, plus commit-qualified companion tags.
- CNB does not deploy, roll back, or otherwise perform CD. DoOps agents promote
  the recorded image digest and own all CD.
